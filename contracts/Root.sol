...
contract Root is IRoot, Collection, BaseMaster, IUpgradable, RandomNonce { // Vault {
    ...
    //string constant PUBLIC_RESOLVER = "addr";
    ...
    event CreatedPrimary(address primary);
    event ChangedPrimary(address oldPrimary, address newPrimary);
    ...
    // function getPrimary() public view responsible returns (address certificate) {
    //     (int32 wid, uint addr) = address(msg.sender).unpack();
    //     string name = format("{}", addr);
    //     string path = name + SEPARATOR + PUBLIC_RESOLVER + SEPARATOR + _tld;
    //     return {value: 0, flag: MsgFlag.REMAINING_GAS, bounce: false} _certificateAddress(path);
    // }
    ...
    function register(
        ...
        //_balance += amount;
        if (!_active) {
            _returnTokens(msg.value, msg.sender, TransferBackReason.IS_NOT_ACTIVE);
            return;
        }
        ...
        if (kind == TransferKind.REGISTER) {
            ...
            (string path, DomainSetup setup, optional(TransferBackReason) error) = _buildRegisterParams(
                msg.value, msg.sender, name, Gas.REGISTER_DOMAIN_VALUE
            );
            if (error.hasValue()) {
                _returnTokens(msg.value, msg.sender, error.get());
                return;
            }
            _deployDomain(path, setup);

            _dao.transfer({value: msg.value - Gas.REGISTER_DOMAIN_VALUE, flag: 0, bounce: false});
            ...
            _dao.transfer({value: msg.value - Gas.RENEW_DOMAIN_VALUE, flag: 0, bounce: false});

        } else {
            _returnTokens(msg.value, msg.sender, TransferBackReason.UNKNOWN_TRANSFER);
        }
    }
    ...
    function reserve(string[] paths, string reason, optional(address) owner) public view override onlyDao cashBack {
        ...
        for (string path : paths) {
            ...
            DomainSetup setup = DomainSetup({
                ...
                expireTime: now + Constants.RESERVED_EXPIRE_TIME,
                amount: 0
            });
            _deployDomain(path, setup);
        }
    ...
    function _buildRegisterParams(
        ...
        if (amount < price + Gas.REGISTER_DOMAIN_VALUE) {
            return ("", empty, TransferBackReason.LOW_MSG_VALUE);
        }
    ...
    function _buildDomainCallParams(
        ...
        _upgradeToLatest(Constants.DOMAIN_SID, domain, _admin, Gas.UPGRADE_SLAVE_VALUE, 0);
    ...
    function _returnTokens(uint128 amount, address recipient, TransferBackReason reason) private {
        ...
        recipient.transfer({value: 0, flag: MsgFlag.ALL_NOT_RESERVED, bounce: false, body: payload});
    }
