...
async function lookupNames(
  ...
  const nfts = await Promise.all(addresses.accounts.map(async (indexAddress) => {
    try {
      ...
      if (address === _nftJson.target) { // we check if the name has already set their target address, you can remove this if you want to get all names that an address own
